...
hive_home := $(addsuffix tools/apache-hive-2.2.0-bin, $(current_dir))
hadoop_home := $(addsuffix tools/hadoop-3.1.0 , $(current_dir))
spark_home := $(addsuffix tools/spark-2.3.0-bin-hadoop2.7, $(current_dir))
JAVA_HOME:= /home/mohamed/Downloads/java/
#########################################
...
download_hadoop:
	...
	cd ${current_dir}tools; wget http://wwwftp.ciril.fr/pub/apache/hadoop/common/hadoop-3.1.0/hadoop-3.1.0.tar.gz && tar -xvf hadoop-3.1.0.tar.gz && rm -rf hadoop-3.1.0.tar.gz

download_spark:
	mkdir -p ${current_dir}tools
	cd ${current_dir}tools; wget http://mirror.ibcp.fr/pub/apache/spark/spark-2.3.0/spark-2.3.0-bin-hadoop2.7.tgz && tar -xvf spark-2.3.0-bin-hadoop2.7.tgz && rm -rf spark-2.3.0-bin-hadoop2.7.tgz

download_hive:
	mkdir -p ${current_dir}tools
	cd ${current_dir}tools; wget http://apache.mediamirrors.org/hive/hive-2.2.0/apache-hive-2.2.0-bin.tar.gz && tar -xvf apache-hive-2.2.0-bin.tar.gz && rm -rf apache-hive-2.2.0-bin.tar.gz
