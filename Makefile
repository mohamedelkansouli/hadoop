...
spark_home := $(addsuffix tools/spark-2.0.0-bin-without-hadoop, $(current_dir))
...
download_spark:
	mkdir -p ${current_dir}tools
	cd ${current_dir}tools; wget https://archive.apache.org/dist/spark/spark-2.0.0/spark-2.0.0-bin-without-hadoop.tgz && tar -xvf spark-2.0.0-bin-without-hadoop.tgz && rm -rf spark-2.0.0-bin-without-hadoop.tgz
